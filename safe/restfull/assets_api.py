...
def get_assets_list(request,format=None):
    ...
    if request.method == 'GET':
            ...
            all_list = []
            for i in all_lists:
                i[3] = Assets.objects.get(assets_status=i[3], id=i[0]).get_assets_status_display()
                i[4] = Business_group.objects.filter(id=i[4]).values_list('business_name', flat=True)[0]
...
def run_assets_asnible(request, id, format=None):
    ...
    if request.method == 'GET':
            ...
            assets_process = ANSRunner(resource)
            assets_user = ANSRunner(resource)
            assets_network.run_model(host_list=[ip], module_name='shell',
                                     module_args="netstat -tlnp")
            assets_process.run_model(host_list=[ip], module_name='shell', module_args="ps -aux")
            assets_user.run_model(host_list=[ip], module_name='shell',
                                  module_args="cat /etc/passwd")
            assets_network = assets_network.get_model_result()
            print assets_user
            assets_process = assets_process.get_model_result()
            assets_user = assets_user.get_model_result()
            SQL_db.assets_ansible(id, assets_info, assets_network, assets_process, assets_user, request.user.id,
                                  ip)
            ret['code'] = 1000
            ret['msg'] = 'ok'
            return JsonResponse(ret)
